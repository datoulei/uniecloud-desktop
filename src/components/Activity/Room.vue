    ...
    <div class="wrap-left">
      <div class="action-bar">
        ...
        <div class="desc-label">
          {{ roomName }}(最多容纳{{ maxPeople }}人)-日期区间分房情况
        </div>
      </div>
      <!-- 拖拽 -->
      <div class="room-wrap">
          ...
          <div v-for="room in useRooms" :key="room.id" class="room">
            ...
            >
              ...
              <a-icon
                type="check-circle"
                theme="filled"
                :class="['check-circle', canShowCheckInIcon(room) ? 'red' : '']"
                @click="checkIn(room)"
              />
            </div>
            <a-input
              ...
              hidden="true"
              :id="`input-${room.id}`"
              @blur="HandleEditFinish(room)"
            ></a-input>

            <draggable
              :value="room.orderItems"
              draggable=".item"
              handle=".item-drag"
              @change="(e) => doAllot(e, room)"
              group="room"
              class="v-draggable"
            >
              <div
                v-for="it in room.orderItems"
                :key="it.id"
                class="item"
                flex="cross:center"
              >
                <span :class="['item-drag', signIn(it)]">
                  <span
                    v-for="attendee in it.attendees"
                    :key="attendee.id"
                    :class="isCheckIn(attendee.OrderAttendee)"
                  >
    ...
    <div class="wrap-right">
      ...
      <div class="book-whole-room">
        ...
        <div class="name-wrap" v-if="roomOrder.length > 0">
          ...
          >
            <div
              v-for="item in roomOrder"
              :key="item.id"
              class="room-item"
              flex="cross:center"
            >
              <span
                class="item-drag name"
                v-for="r in item.attendees"
                :key="r.id"
                :order-id="item.id"
  ...
  created() {
    this.getRoomType().then((rst) => {
      if (rst.rows.length > 0) {
        ...
        this.allowOrderBeginDate = moment(
          this.conditions[0].allowOrderBeginDate,
          'YYYY-MM-DD'
        );

        this.allowOrderEndDate = moment(
          this.conditions[0].allowOrderEndDate,
          'YYYY-MM-DD'
        );
  ...
  methods: {
    ...
    load(type) {
      if (type === 'roomType') {
        if (Array.isArray(this.conditions)) {
          const selectedRoomType = this.conditions.filter(
            (item) => this.checkType === item.id
          );

          this.allowOrderBeginDate = moment(
            selectedRoomType[0].allowOrderBeginDate,
            'YYYY-MM-DD'
          );

          this.allowOrderEndDate = moment(
            selectedRoomType[0].allowOrderEndDate,
            'YYYY-MM-DD'
          );
          this.values = [this.allowOrderBeginDate, this.allowOrderEndDate];
        }
    ...
    async search() {
      try {
        ...
        const result = await this.$http.get(url, {
          params: {
            checkIn: this.values[0]
              ? dayjs(this.values[0]).format('YYYY-MM-DD')
              : '',
            checkOut: this.values[1]
              ? dayjs(this.values[1]).format('YYYY-MM-DD')
              : '',
    ...
    disabledDate(current) {
      return (
        current <
          moment(
            dayjs(this.allowOrderBeginDate)
              .subtract(1, 'day')
              .format('YYYY-MM-DD')
          ).endOf('day') ||
