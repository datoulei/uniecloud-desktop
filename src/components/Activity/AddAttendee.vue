    ...
    <div class="content-wrap m-t-24 flex-box">
      <a-form :form="form" @submit="handleSubmit">
        <a-form-item label="请选择票种" class="cover-form-label">
          <a-select @change="(value) => getTicket(value)" class="long-input" v-decorator="['ticketId', { rules: [{ required: true, message: '请选择' }] }]">
        ...
        <!-- <a-form-item label="请上传头像" class="cover-form-label" v-if="ticketInfo.showAvatar">
          <FileUploader
            :face="activityInfo.enableFaceDetect"
            v-decorator="[
              'avatar',
              {
                rules: [
                  {
                    required: activityInfo.enableFaceDetect,
                    message: '请上传头像'
                  }
                ]
              }
        ...
        <a-form-item label="身份证" class="cover-form-label" v-if="ticketInfo.showIdCard">
          <a-input
            class="long-input"
        ...
        <a-form-item label="单位" class="cover-form-label" v-if="ticketInfo.showWork">
          <a-input
            class="long-input"
            placeholder="请输入单位"
            v-decorator="[
        ...
        <a-form-item label="职位" class="cover-form-label" v-if="ticketInfo.showJob">
          <a-input
            class="long-input"
            placeholder="请输入职位"
            v-decorator="[
        ...
        <a-form-item label="性别" v-if="ticketInfo.showSex">
          <a-radio-group
            v-decorator="[
              `sex`,
              {
        ...
        <a-button class="m-l-120" type="primary" html-type="submit" :loading="creating">保存</a-button>
      </a-form>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    handleSubmit(e) {
      ...
      this.form.validateFields(async (err, formData) => {
        if (!err) {
          let values = []
          const filterArray = ['ticketId', 'groupId', 'inviteCode', 'avatar', 'name', 'mobile', 'work', 'sex', 'job']
          ...
          try {
            await this.$http.post(`/activities/${this.activityId}/attendees`, {
              ...orginFormData,
              values,
              origin: 4,
            })
