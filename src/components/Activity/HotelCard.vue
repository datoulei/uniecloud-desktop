    ...
    <div class="hotel-info">
      ...
      <div class="cover">
        <img
          v-if="hotel.cover"
          :src="hotel.cover"
          style="width: 208px; height: 122px;"
        />
      </div>
      <!-- 酒店的基本信息 -->
      <div class="info">
        ...
        <div class="item-1">
          ...
          ><span class="c-1">{{ hotel.address }}</span>
        </div>
        <div class="desc-info">
      ...
      <div class="operation-wrap">
        ...
        <div class="operation">
          <span class="room-type-label">
            {{ statistics.list ? statistics.list.length : 0 }}个房型
          </span>
      ...
      <div class="other-info-count">
        <div style="display: flex; flex-diretion: row; align-items: center;">
          ...
          <span class="span-count">
            总房间数:{{ statistics.roomTotal }} &nbsp; 已预定房间数:{{
              statistics.roomUse
            }}
            &nbsp; 剩余房间数:{{ statistics.roomFree }} &nbsp; 已分配人数:{{
              statistics.guestTotal
            }}
            &nbsp; 已入住人数:{{ statistics.guestCheckIn }} &nbsp; 未入住人数:{{
              statistics.guestNoCheckIn
            }}
          </span>
        </div>
        <div class="btn-wrap">
          <a
            @click="HandleShow(hotel.id)"
            :id="`label${hotel.id}`"
            style="color: #f60f0f; display: inner-block;"
            >展开</a
          >
        </div>
      </div>
...
export default {
  ...
  created() {
    this.HandleSubmit(
      this.hotel.id,
      moment(this.hotel.orderBeginDate).utcOffset(480).format('YYYY-MM-DD')
    );
  },
    ...
    async HandleSubmit(hotelId, date) {
      try {
        const rst = await this.$http.get(
          `/activities/${this.activityId}/hotels/${hotelId}/statistics`,
          {
            params: {
              date: `${date}`
            }
