      ...
      <div slot="action" slot-scope="text, record" class="action-div">
        <span>
          <img
            src="~@/assets/查看.png"
            class="table-icon"
            @click="handleViewAttendeeDetail(record)"
        /></span>
        <span>
          <img
            src="~@/assets/编辑.png"
        ...
        <span>
          <img
            src="~@/assets/删除.png"
            class="table-icon"
            @click="handleDeleteAttande(record)"
          />
        </span>
      </div>
    </a-table>
...
export default {
  ...
  methods: {
    ...
    async getAttendeeList() {
      ...
      // 离线
      if (this.useType === 2) {
        attendees = await offlinedb
          .getDB("attendees", this.siteId)
          .get("attendeeList")
          .value();
        if (this.keyword) {
