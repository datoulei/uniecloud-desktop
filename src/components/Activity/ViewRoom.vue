<template>
  <div class="wrap">
    <div class="wrap-top">
      ...
      <div class="date">
        ...
        <div class="date-group">
          <a-radio-group v-model="checkDate" buttonStyle="solid" @change="getRoomList">
            <a-radio-button :value="d.date" v-for="d in dateList" :key="d.date">{{ d.date }}</a-radio-button>
          </a-radio-group>
        </div>
      </div>
    ...
    <p v-for="s in countInfo" :key="s.name" style="color: #666666; font-size: 12px; margin-top: 10px;">
    ...
    <div class="wrap-body">
      <div v-for="r in room" :key="r.id" class="room">
        <div :class="['room-name', (isLock(r.items) || r.orderItems.length > 0) && 'had-sharking-room-name']" @dblclick="HandleEdit(r)" :id="`div-${r.id}`">
          {{ r.name }}
          <div class="check-circle-wrap">
            <a-icon type="check-circle" theme="filled" :class="['', canShowCheckInIcon(r) ? 'check-circle' : '']" />
          </div>
        </div>
        <a-input class="room-input" hidden="true" :id="`input-${r.id}`" @blur="HandleEditFinish(r)"></a-input>
        <div class="room-body" v-if="r.orderItems.length > 0">
          <div v-for="item in r.orderItems" :key="item.id" class="item">
            <span v-for="attendee in item.attendees" :key="attendee.id" :class="isCheckIn(attendee)">{{ attendee.name }}</span>
          </div>
        </div>
        <div class="lock-wrap" v-else>
          <div class="lock-wrap-content">
            <img v-if="isLock(r.items)" src="~@/assets/lock.png" style="width: 32px; height: 32px;" @click="HandleLock(r)" />
            <img v-else src="~@/assets/unlock.png" style="width: 32px; height: 32px;" @click="HandleLock(r)" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    dateList() {
      let dateList = this.conditions.filter((item) => item.id === this.checkType)
      let dateArray = []
  ...
  methods: {
    ...
    async getCountInfo() {
      try {
        const result = await this.$http.get(`/activities/${this.activityId}/hotels/${this.hotelId}/statistics`, {
          params: {
            date: this.checkDate,
          },
        })
        const roomType = this.type.filter((item) => item.id === this.checkType)[0].name
    ...
    async HandleLock(room) {
      ...
      try {
        await this.$http.put(`/activities/${this.activityId}/hotels/${this.hotelId}/room_types/${this.checkType}/rooms/${room.id}`, {
          date: this.checkDate,
          isLock: !isLock,
        })
        this.getRoomList()
    ...
    async getRoomList() {
      try {
        this.room = await this.$http.get(`/activities/${this.activityId}/hotels/${this.hotelId}/room_types/${this.checkType}/rooms`, {
          params: {
            date: this.checkDate,
          },
        })
