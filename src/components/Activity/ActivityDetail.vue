  ...
  <div class="root">
    <div class="root-child-1">
      <a-card class="a-card-1">
        ...
        <div
          class="view-sign-text"
          v-if="accoutType === 1 || this.UserInfo.enableAttendeeManage === true"
        >
          <img src="~@/assets/sign.png" />
          <span @click="HandleToAttendeeList('')" style="cursor: pointer"
            >查看签到名单</span
          >
        </div>
      </a-card>
      <a-card class="a-card-2">
        ...
        <div class="direct-sign-div" v-if="buttonGroup === 1">
          ...
          <div>
            ...
            <span class="font-span">
              <img src="~@/assets/unconnect.png" />身份证读卡器
            </span>
          </div>
          <!-- 签到栏 -->
      ...
      <!-- 检索签到弹框 -->
      <SignModal
        :visible="searchSignModal"
        :title="searchModalTittle"
        width="80%"
        @cancel="handleClose"
        :maskClosable="false"
        :destroyOnClose="true"
      >
        ...
        >
          ...
          <a-button
            type="primary"
            slot-scope="attendees"
            slot="action"
            :style="{ 'margin-left': '16px' }"
            @click="HandleOnSign(attendees.mobile)"
            >签到</a-button
          >
        </a-table>
      ...
      >
        ...
        >
          <div class="sign-result">
            <div class="sign-result-header">
              <div class="sign-result-header-userinfo">
                ...
                <div class="sign-result-header-userinfo-content">
                  <span class="sign-result-header-userinfo-content-username">{{
                    signAttendee.name
                  }}</span>
                  <br />
                  <span class="sign-result-header-userinfo-content-mobile">{{
                    hiddenMobile(signAttendee.mobile)
                  }}</span>
                </div>
              </div>
              ...
              <div class="sign-result-header-content">
                {{
                  signStatus > 0
                    ? signStatus === 1
                      ? "已签到"
                      : signAttendee.signRecord &&
                        signAttendee.signRecord.length > 1
                      ? "重复签到"
                      : "签到成功"
                    : "未签到"
                }}
            ...
            <!-- 签到显示信息 -->
            <div
              :class="['sign-result-body', signStatus === 2 ? 'm-b-24' : '']"
            >
              <div
                v-for="item in afterSignFields"
                :key="item.key"
                class="sign-column"
              >
            ...
            <div
              class="bind-xcard"
              v-if="signStatus === 2 && setting.enableRfid"
            >
              <div>
          ...
          <div
            ...
            slot="footer"
            slot-scope="{ ok }"
            v-if="signStatus !== 2 && setting.signByFace"
          >
            <a-button
              class="a-button"
              type="primary"
              @click="HandleSignAndCollection"
              >签到并采集人像</a-button
            >
...
export default {
      ...
      if (result) {
        ...
        if (this.setting.enableExtraScreen) {
          this.getExtraScreenTemplate().then((rst) => {
            if (rst) {
              this.buildTemplate(
                rst.template.welcomePageData.elements,
                this.activityInfo
              ).then((_rst) => {
              ...
              // 离线模式，直接从文件中读取
              if (this.useType === 2) {
                rst.template.welcomePageData.page.bgImg = offlinedb
                  .getDB("extraScreen")
                  .get("welcomebg")
                  .value();
  ...
  computed: {
    ...
    afterSignFields() {
      return ActivityDetail.buildSignFields(
        this.afterSignColumn,
        this.activityFields,
        this.signAttendee
      );
    },
  ...
  methods: {
          ...
          if (result) {
            ...
            if (this.setting.autoConfirmSign) {
              const isNotAllowRepeatSign =
                !this.setting.allowRepeatSign &&
                Array.isArray(this.signAttendee.signRecord) &&
                this.signAttendee.signRecord.length > 0;
              ...
              this.signAccount(
                this.signAttendee.id,
                Number(this.currentSite.id),
                this.signAttendee
              ).then((result) => {
                if (result) {
                  this.signAttendee = result;
          ...
          return new Promise((resolve) => {
            if (ele.type === "label") {
              ...
              if (data.avatar) {
                ...
                if (this.useType === 2) {
                  ele.value = offlinedb
                    .getDB(data.id, this.currentSite.id)
                    .get("base64")
                    .value();
                  resolve(ele);
                }
              ...
              data.fieldValues.map((fieldvalue) => {
                if (ele.type === fieldvalue.fieldId) {
    ...
    async uploadImage(uri) {
      if (this.detectType === 1) {
        ...
        try {
          ...
          this.$message.error(
            "签到成功,但采集人像信息失败:" + error.response.data.message
          );
        }
        this.faceSign = false;
        this.signStatus = 2;
          ...
          this.getAttendeeInfo(img_uri).then((result) => {
            ...
            if (result) {
              ...
              if (this.setting.autoConfirmSign) {
                ...
                this.faceSign = false;
                this.signStatus =
                  this.signAttendee.signRecord.length === 0 ? 0 : 1;
                this.signResult = true;
              }
            } else {
              this.faceSign = false;
    ...
    async getSignData(sId) {
      try {
        ...
        // 离线模式
        if (this.useType === 2) {
          const statistics = await offlinedb
            .getDB("statistics", siteId)
            .get("statistics")
            .value();
          console.log(JSON.stringify(statistics));
          this.signStatistics.total = statistics.attendeeTotal;
          this.signStatistics.signCount = statistics.attendeeSignTotal;
          this.signStatistics.percent =
            statistics.attendeeTotal > 0
              ? Math.round(
                  (statistics.attendeeSignTotal / statistics.attendeeTotal) *
                    100
                )
              : 0;
          return;
        ...
        this.signStatistics.percent =
          result.attendeeTotal > 0
            ? Math.round(
                (result.attendeeSignTotal / result.attendeeTotal) * 100
              )
            : 0;
      } catch (error) {
    ...
    async getAttendeeInfo(keyword) {
      if (this.useType === 2) {
        // 离线模式
        let attendeeList = await offlinedb
          .getDB("attendees", this.currentSite.id)
          .get("attendeeList")
          .value();
        if (!attendeeList) {
          attendeeList = [];
        ...
        if (result.length > 0) {
          let signList = await offlinedb
            .getDB("userSignList", this.currentSite.id)
            .get("list")
            .value();
          let tempArr = [];
          if (Array.isArray(signList)) {
            tempArr = signList.filter(
              (item) => item.attendeeId === result[0].id
            );
          }
      ...
      try {
        return await this.$http.get(
          `/activities/${this.activityId}/site/${this.currentSite.id}/attendee`,
          {
            params: {
              keyword,
              silent: true, // 静默标识
    ...
    async signAccount(attendeeId, siteId, signAttendee) {
      try {
        ...
        if (this.useType === 2) {
          let signList = [];
          let bool = await offlinedb
            .getDB("userSignList", this.currentSite.id)
            .has("list")
            .value();
          if (bool) {
            signList = await offlinedb
              .getDB("userSignList", this.currentSite.id)
              .get("list")
              .value();
          }
          ...
          let tempArr = signList.filter(
            (item) => item.attendeeId === signAttendee.id
          );
    ...
    async bindRFID(RFID) {
      ...
      if (this.useType === 2) {
        ...
        await offlinedb
          .getDB("attendees", this.currentSite.id)
          .set("attendeeList", tempArr)
          .write();
        let rfids = [];
        let ids = await offlinedb
          .getDB("rfid", this.currentSite.id)
          .get("list")
          .value();
        if (ids) {
          rfids = ids;
    ...
    async getAttendeeList(pkw) {
      try {
        // 离线模式
        if (this.useType === 2) {
          const attendeeList = await offlinedb
            .getDB("attendees", this.currentSite.id)
            .get("attendeeList")
            .value();
          let attendees = [];
          ...
          if (pkw) {
            attendees = attendeeList.filter((attendee) => {
              if (
                ...
                attendee.avatar = offlinedb
                  .getDB(attendee.id, this.currentSite.id)
                  .get("base64")
                  .value();
    ...
    toExtraScreen(attendeeInfo) {
      this.getExtraScreenTemplate().then((result) => {
        if (result) {
          ...
          this.buildTemplate(template.elements, attendeeInfo).then(
            async (rst) => {
              template.elements = rst;
              // 离线模式，直接从文件中读取
              if (this.useType === 2) {
                template.page.bgImg = await offlinedb
                  .getDB("extraScreen")
                  .get("signbg")
                  .value();
    ...
    async getCardTemplate() {
      ...
      try {
        return await this.$http.get(
          `/activities/${this.activityId}/sign-sites/${this.currentSite.id}/card`
        );
      } catch (error) {
    ...
    printCard() {
      const cardData = this.cardTemplate;
      if (cardData !== null) {
        this.buildTemplate(cardData.template.elements, this.signAttendee).then(
          (result) => {
            cardData.template.elements = result;
            const pw = Math.ceil(cardData.template.page.input.width);
            const ph = Math.ceil(cardData.template.page.input.height);
    ...
    async handleGetSiteInfo() {
      try {
        if (this.useType === 2) {
          const rst = await offlinedb
            .getDB("siteInfo", this.currentSite.id)
            .get("info")
            .value();
          this.afterSignColumn = rst.fields;
        }
        const rst = await this.$http.get(
          `/activities/${this.activityId}/sign-sites/${this.currentSite.id}`
        );
        this.afterSignColumn = rst.fields;
      } catch (error) {}
    },
