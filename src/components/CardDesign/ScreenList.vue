<template>
  <div>
    ...
    <div class="list">
      <div :class="index % 3 === 0 ? 'screen' : 'screen card-margin'" v-for="(screen, index) in screenList" :key="screen.id">
        <div class="cover">
  ...
  methods: {
    ...
    deleteScreen(screen) {
      this.$confirm({
        ...
        content: '确定要删除此拓展屏?',
        onOk: async () => {
          await this.$http.delete(`/activities/${this.activityId}/cards/${screen.id}`)
          this.screenList = await this.getScreenList()
        },
        onCancel() {},
      })
    ...
    // 拓展屏列表
    async getScreenList() {
      const result = await this.$http.get(`/activities/${this.activityId}/cards`, {
        query: {
          type: 1,
        },
      })
    ...
    // 签到地点列表
    async getSiteList() {
      const result = await this.$http.get(`/activities/${this.activityId}/sign-sites`)
      return result
    },
    HandleCancel() {
      this.isShow = false
    ...
    async HandleOk() {
      try {
        await this.$http.put(`/activities/${this.activityId}/cards/${this.cardId}`, {
          siteId: this.siteId,
        })
